...
class RNN3:
    ...
    def backProp(self,node,error=None):
        ...
        self.db2 += deltas
        deltas = np.dot(self.W2.T, deltas)#(d,dm)(dm)=>(d,)

        deltas *= (node.hActs1 != 0)#(d,)

        if node.isLeaf:
            self.dL[node.word] += deltas
        else:
            
            self.dW1 += np.outer(deltas, np.hstack([node.left.hActs2, node.right.hActs2]))#(d,)outer(2dm,)=>(d,2dm)
            self.db1 += deltas#(d)
            deltas = np.dot(self.W1.T, deltas)#(2dm,d)(d)=>(2dm)
