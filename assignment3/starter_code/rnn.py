...
class RNN:
    ...
    def __init__(self,wvecDim,outputDim,numWords,mbSize=30,rho=1e-4):
        self.wvecDim = wvecDim#30
        self.outputDim = outputDim#5
    ...
    def forwardProp(self,node,correct=[], guess=[]):
        ...
        # if haven't finished doing forward prop on the left child, do it
            ...
            node.hActs1 = np.dot(self.W, np.hstack([node.left.hActs1, node.right.hActs1])) + self.b#(d,2d)(2d,)+(d,)
            node.hActs1[node.hActs1 < 0] = 0

        node.probs = np.dot(self.Ws, node.hActs1) + self.bs#(5,d)(d,)+(5,)=>(5,)
    ...
    def backProp(self,node,error=None):
        ...
        self.dWs += np.outer(dsoftmax, node.hActs1)#(5,)outer(d,)=>(5,d)
        self.dbs += dsoftmax#(5,)
        dh = np.dot(self.Ws.T, dsoftmax)#(d,5)(5,)=>(d,)
        ...
        if node.isLeaf:
            ...
            self.dW += np.outer(dh, np.hstack([node.left.hActs1, node.right.hActs1]))#(d,)outer(2d,)=>(d,2d)
            self.db += dh

            dh = np.dot(self.W.T, dh)#upper loss grad on lower h
            #pdb.set_trace()
            self.backProp(node.left, dh[:self.wvecDim])#left child backprop with error from parent
            self.backProp(node.right, dh[self.wvecDim:])#right child backprop with error from parent
